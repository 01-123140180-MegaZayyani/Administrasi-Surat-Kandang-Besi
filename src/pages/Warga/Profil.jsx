...
export default function Profil() {
  ...
  useEffect(() => {
    const savedData = localStorage.getItem("profil");
    if (savedData) {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await api.put("/api/auth/profil", {
        ...formData,
        id: userData.id 
      });
  ...
  if (!userData) {
    return <div className="p-10 text-center font-black">Memuat Profil...</div>;
  }
              ...
              {isEditing ? (
                <div className="flex gap-4">
                  <button 
                    ...
                    className="bg-green-600 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all"
                  >
                    Simpan
                  </button>
                  <button 
                    onClick={() => { setIsEditing(false); setFormData(userData); }} 
                    className="bg-slate-100 text-slate-400 px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest"
                  >
                    Batal
                  </button>
                </div>
                ...
                <button 
                  ...
                  className="bg-[#1E3A8A] text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                >
                  Edit Profil
                </button>
              )}
